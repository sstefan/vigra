...
struct LocalMinTests : TestImageReader {
...
#if 0
	// again the same thing: does it also make the code above "faster"? -> No.
	{
	  cout << "Timing 3D (new, list-based, via BGL (vertex_desc=coord), direct propmaps src/dest): " << endl;
	  TICTOCLOOP_BEGIN(repetitions,best_of_repetitions)
	      localMinMaxGraph3(ggv,
	...
	bool flag = false; // this suffices to compile the below code but not run it, which makes the above run faster; in fact, the code is not even necessarily included in the output (e.g. the string is not in the assembly file), apparently it has been optimized away anyways.
	...
	// doNothing(&flag); // effectively prevents optimizing away of the body below:
	if (flag)
	{
